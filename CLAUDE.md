...
`make duplication-check` runs jscpd (v5, the Rust engine) over `backend/src` and
`backend/tests` and reports every copy-pasted block of 5+ lines / 50+ tokens.
It exits 1 once duplicated lines pass the `threshold` in `backend/.jscpd.json`.
Alembic versions are excluded — autogenerated migrations are near-identical by
construction. The frontend equivalent is `npm run duplication-check` in
`frontend/`.

Output is the compact `ai` reporter, one line per clone. Add `--reporters
console` for the detailed view with columns and token counts, and narrow the
scan to a path when you only care about one area:
...
### The duplication Stop hook

`.claude/hooks/check-duplication.sh` runs per turn when backend `.py` or
frontend `.ts`/`.tsx` files are dirty, and sends the turn back when either:

- a clone's lines overlap lines the turn changed — the copy-paste you just made,
  caught while the context is still loaded; or
- the ratchet threshold is passed, which is what CI fails on.

Clones that were already there before the turn stay quiet, even in a file you
edited: the test is against changed *lines*, not changed files. So a finding is
about code this turn wrote. Extract the shared part into a helper both sides
call. If the duplication is genuinely the better design, say why and finish —
the hook fires at most once per turn and will not block a second stop.

There is no jscpd MCP server wired up on purpose: it ships as a separate daemon
running the older v4 engine, so its numbers can disagree with the v5 CLI that
the hook and CI run.
