...
# a new migration + full backfill by design, not an in-place alter. The literal
# is repeated rather than imported: migrations are frozen snapshots and must not
# depend on application code that will keep moving.
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        # its embedding orphaned -- and deletion often bypasses application code
        # entirely, cascading from a book. These three columns exist purely as
        # cascade anchors: exactly one is set, it always equals content_id
        # (enforced by the CHECK below), and the database prunes the row when its
        # source goes.
        sa.Column(
            "note_id",
            sa.Integer(),
        ...
        #
        # Each branch tests IS NOT NULL before comparing, and that is load
        # bearing rather than belt-and-braces. Comparing a NULL anchor to
        # content_id yields NULL, so the whole disjunction yields NULL, and SQL
        # takes any CHECK that is not false as satisfied -- which would admit
        # precisely the row this constraint exists to forbid: one that no
        # foreign key can cascade and that the orphan sweep, which reads the
        # anchor, cannot see either.
        sa.CheckConstraint(
            "(content_type = 'note' AND note_id IS NOT NULL AND note_id = content_id "
            "AND highlight_id IS NULL AND digest_id IS NULL) OR "
            "(content_type = 'highlight' AND highlight_id IS NOT NULL "
            "AND highlight_id = content_id "
    ...
    # delete would otherwise seq-scan this table.
    op.create_index("ix_embeddings_user_id", "embeddings", ["user_id"])
    op.create_index("ix_embeddings_book_id", "embeddings", ["book_id"])
    # Partial: each anchor is set on only the rows of its own content type, so
    # indexing the NULLs would be dead weight. The ORM declares the same
    # postgresql_where, or autogenerate would report a diff forever.
