...
async def test_sessions_are_summed_across_the_book(
    ...
    await add_session(db_session, test_book, datetime(2024, 1, 1, 20, tzinfo=UTC), minutes=30)
    await add_session(db_session, test_book, datetime(2024, 1, 5, 20, tzinfo=UTC), minutes=10)
...
async def test_sessions_of_another_book_or_another_user_are_excluded(
    ...
    await add_session(db_session, test_book, datetime(2024, 1, 1, 20, tzinfo=UTC), minutes=30)
    await add_session(db_session, other_book, datetime(2024, 1, 2, 20, tzinfo=UTC), minutes=30)
    await add_session(
        db_session,
        test_book,
        datetime(2024, 1, 3, 20, tzinfo=UTC),
        minutes=30,
...
async def test_progress_is_measured_from_the_latest_sessions_end_position(
    ...
    await add_session(
        db_session, test_book, datetime(2024, 1, 1, 20, tzinfo=UTC), end_position=[20, 0]
    )
    await add_session(
        db_session, test_book, datetime(2024, 1, 2, 20, tzinfo=UTC), end_position=[150, 0]
    )
