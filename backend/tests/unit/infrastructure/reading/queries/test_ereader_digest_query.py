"""Tests for the ereader chapter-digest read model."""
...
@pytest.fixture
def query(db_session: AsyncSession) -> EreaderDigestQuery:
    """The query service wired the way the container wires it."""
    return EreaderDigestQuery(db=db_session)
...
async def add_digest(
    ...
    """Generate digest for a chapter."""
    content = ChapterDigest(
...
async def test_unknown_client_book_is_distinguished_from_one_without_digest(
    query: EreaderDigestQuery, synced_book: Book, add_chapter: AddChapter
) -> None:
    await add_chapter(synced_book, "No digest", chapter_number=1)
...
async def test_another_users_book_is_invisible(
    ...
    await add_digest(db_session, chapter)

    assert await query.list_for_client_book(CLIENT_BOOK_ID, UserId(other_user.id)) is None


async def test_only_chapters_with_digest_are_listed_in_chapter_order(
    query: EreaderDigestQuery,
    db_session: AsyncSession,
    synced_book: Book,
    add_chapter: AddChapter,
    ...
    await add_digest(db_session, third)
    await add_digest(db_session, first)
...
async def test_only_question_text_is_exposed_alongside_summary_and_keypoints(
    ...
    chapter = await add_chapter(synced_book, "One", chapter_number=1)
    content = await add_digest(db_session, chapter, summary="What this chapter covers")
...
async def test_a_nested_chapter_carries_its_parents_name(
    ...
    await add_digest(db_session, nested)
...
async def test_digest_of_another_book_is_excluded(
    query: EreaderDigestQuery,
    db_session: AsyncSession,
    synced_book: Book,
    add_chapter: AddChapter,
    ...
    await add_digest(db_session, mine)
    await add_digest(db_session, theirs)
