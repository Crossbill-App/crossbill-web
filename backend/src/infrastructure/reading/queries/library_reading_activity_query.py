...
``LibraryReadingActivityCalculator`` -- which owns what they add up to -- and
then looks up the titles of the books that ended up on the grid. Nothing here
decides which day a session belongs to, or how dark a square is.
"""
...
class LibraryReadingActivityQuery:
    ...
    def __init__(
        self, db: AsyncSession, activity_calculator: LibraryReadingActivityCalculator
    ) -> None:
        self.db = db
    ...
    async def get_activity(
        ...
        return LibraryReadingActivityView(activity=activity, titles=titles)
