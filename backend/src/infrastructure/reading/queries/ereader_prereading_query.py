"""Query adapter for the ereader's chapter-prereading list.
...
them to the prereading rows in Python. Here one join returns exactly the
chapters that have prereading, with the parent chapter's name alongside.
"""
...
from src.application.reading.queries.ereader_prereading import EreaderChapterPrereadingView
...
from src.infrastructure.reading.orm.chapter_prereading_content_model import (
    ChapterPrereadingContent as PrereadingORM,
)


class EreaderPrereadingQuery:
    """Serves the ereader prereading list from one join per book."""
    ...
    async def list_for_client_book(
        ...
        if book_id is None:
            return None
        return await self._fetch_prereading(book_id)
    ...
    async def _fetch_prereading(self, book_id: int) -> tuple[EreaderChapterPrereadingView, ...]:
        """Load each chapter that has prereading, with its parent chapter's name.
        ...
        stmt = (
            select(
                ...
                PrereadingORM.summary,
                PrereadingORM.keypoints,
                PrereadingORM.questions,
                PrereadingORM.generated_at,
            )
            .join(PrereadingORM, PrereadingORM.chapter_id == ChapterORM.id)
            .outerjoin(
                parent,
