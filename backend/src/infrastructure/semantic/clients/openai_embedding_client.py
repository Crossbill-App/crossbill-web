...
# OpenRouter caps a single /embeddings request at 96 inputs, so `embed` splits
# anything longer across requests.
#
# This is the client's guarantee, not a contract with its callers: they pass
# whatever a unit of work happens to contain and are entitled not to know the
# provider's limit, which is a fact about OpenRouter rather than about
# embedding. Callers that stay under it on their own do not make the split
# dead -- they make it unexercised, which is a different thing, and the one
# that stops being true the moment a caller batches more aggressively.
