...
class Embedding(Base):
    ...
    Everything the migration creates is mirrored here so ``autogenerate`` reports
    no diff, with one deliberate exception: the HNSW index. Declaring it would
    make ``create_all`` try to build an HNSW index on SQLite, which cannot, so it
    stays migration-only and ``autogenerate`` will always offer to drop it.
    """
    ...
    __table_args__ = (
        ...
        # Byte-identical to migration 062. The IS NOT NULL tests are what make
